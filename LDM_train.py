...
def parse_args():
    ...
    parser.add_argument('--root_path', default=None, help='Dataset root; auto-set by --data_name if omitted')
    parser.add_argument('--task',      default='train', choices=['train', 'test'])
    parser.add_argument('--bs',        default=8,   type=int)
    parser.add_argument('--image_size',default=256, type=int)
    parser.add_argument('--image_show',default=False)
    # DataLoader
    parser.add_argument('--shuffle',     default=True)
    parser.add_argument('--num_workers', default=0, type=int, help='0 for debug; 4 for training')
    parser.add_argument('--pin_memory',  default=True,help='Enable pinned memory for faster GPU transfer; '
                                                                         'OOM->False')

    # EMBedder ---------------------------------------------------------------------------------------------------------
    parser.add_argument('--emb_target',  default='cls', choices=['cls', 'descript'])
    parser.add_argument('--emb_dims',    default=256, type=int, help='nn.Embedding output dim')
    parser.add_argument('--emb_out_dim', default=512,   type=int, help='Projection output dim; must match UNet context_dim')
    ...
    parser.add_argument('--verbose',              default=False, help='Print block shapes at each level')
    parser.add_argument('--vae_ckpt',
                        ...
                        help='Path to pretrained VAE checkpoint (.pt / .pth); if not use, default=None')
    ...
    # UNet: Attention
    parser.add_argument('--unet_num_heads',         default=-1,         help='if not num_head_channels is -1, num_head = unet_ch//num_head_channels')
    parser.add_argument('--unet_num_head_channels', default=8,          type=int)
    parser.add_argument('--use_spatial_transformer',default=True)
    parser.add_argument('--transformer_depth',      default=1,          help='Number of transformer layers')
    ...
    parser.add_argument('--scale_by_std',   default=False, help='Auto-compute scale_factor from first-batch latent std')
    parser.add_argument('--use_ema',        default=True, help='Enable exponential moving average (EMA) of model weights for more stable training and improved inference.')

    # <STAGE 2> Pre-Training ---------------------------------------------------------------------------------------------------------
    parser.add_argument('--epochs',     default=100, type=int)
    parser.add_argument('--lr',         default=1e-4, type=float)
    parser.add_argument('--save_dir',   default='/home/hjchoi/PycharmProjects/PrivaText-CXR/checkpoints/ldm')
    parser.add_argument('--save_every', default=10,  type=int, help='Save checkpoint every N epochs')
    parser.add_argument('--log_every',  default=50,  type=int, help='Print training loss every N steps')
    parser.add_argument('--resume_ckpt',default=None, help='LDM checkpoint to resume from (.pt / .pth)')

    # ------------------------------------------------------------------------------------------------------------------
    # <STAGE 3> DP Fine-tuning
    # ------------------------------------------------------------------------------------------------------------------
    parser.add_argument('--dp_use', default=True, type=bool)
...
def main():
    ...
    if args.root_path is None:
        if args.data_name == 'nih':
            ...
            raise ValueError('--root_path must be set for Mimic-datasets')
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        if (epoch + 1) % args.save_every == 0:
            ...
            print(f'  [ckpt] saved ¡æ {ckpt_path}')
