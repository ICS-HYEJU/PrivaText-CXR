...
full loop works:
    ...
    description (str)
      ¦¦¦¡ BioBERTEmbedder ¡æ context [B, seq_len, 512]
           ¦¦¦¡ LatentDiffusionDP.sample(c)  (DDPM reverse diffusion)
                ¦¦¦¡ latent z ¡æ decode_first_stage(z) ¡æ VAE.decode
                     ¦¦¦¡ image [B,1,256,256]
...
def _to_uint8(img: torch.Tensor) -> np.ndarray:
    """[1,H,W] or [H,W] in [-1,1] ¡æ uint8 [H,W]."""
...
def save_outputs(images, descriptions, sample_idx, output_dir):
    ...
    try:
        ...
        plt.close(fig)
        print(f'[save] grid ¡æ {grid_path}')
    except ImportError:
        print('[save] matplotlib not available ? skipped grid_all.png')

    print(f'[save] {images.shape[0]} image(s) ¡æ {samples_dir}')
    print(f'[save] mapping ¡æ {os.path.join(output_dir, "descriptions.csv")}')
...
def build_and_load_ldm(args, device):
    ...
    if args.lora_ckpt:
        ...
        merge_lora(ldm)                        # fold ¥ÄW into base for sampling
        ldm.to(device)
