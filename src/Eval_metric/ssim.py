"""
Eval_metric/ssim.py  ?  SSIM evaluation for VAE reconstruction
==============================================================
...
3. Compute SSIM between real and reconstructed images per sample
4. Report mean / std / min / max SSIM

SSIM is implemented in pure PyTorch (no external metric library needed).
Formula:
    SSIM(x, y) = (2¥ì_x ¥ì_y + C1)(2¥ò_xy + C2)
               / (¥ì_x©÷ + ¥ì_y©÷ + C1)(¥ò_x©÷ + ¥ò_y©÷ + C2)

    C1 = (k1 * L)©÷,  C2 = (k2 * L)©÷
    k1 = 0.01,  k2 = 0.03,  L = data range

Dependencies: torch, torchvision, numpy  (all standard)

Usage:
    python Eval_metric/ssim.py \\
...
# =============================================================================
# SSIM  (pure PyTorch, no external library)
# =============================================================================
...
def _gaussian_kernel(kernel_size: int = 11, sigma: float = 1.5,
    ...
    Returns:
        Tensor [1, 1, kernel_size, kernel_size]
    """
    coords = torch.arange(kernel_size, dtype=torch.float32,
                          device=device) - kernel_size // 2
...
def compute_ssim_map(img1: torch.Tensor, img2: torch.Tensor,
                     kernel_size: int = 11, sigma: float = 1.5,
                     data_range: float = 2.0) -> torch.Tensor:
    """
    Per-pixel SSIM map between two single-channel image tensors.
    ...
    Returns:
        ssim_map : [B, 1, H, W]  per-pixel SSIM values
    """
    C1 = (0.01 * data_range) ** 2
    C2 = (0.03 * data_range) ** 2

    kernel  = _gaussian_kernel(kernel_size, sigma, device=img1.device)
    padding = kernel_size // 2

    # Local means
    mu1 = F.conv2d(img1, kernel, padding=padding)
    mu2 = F.conv2d(img2, kernel, padding=padding)
    ...
    sigma1_sq = F.conv2d(img1 * img1, kernel, padding=padding) - mu1_sq
    sigma2_sq = F.conv2d(img2 * img2, kernel, padding=padding) - mu2_sq
    sigma12   = F.conv2d(img1 * img2, kernel, padding=padding) - mu1_mu2

    # SSIM map
    numerator   = (2.0 * mu1_mu2 + C1) * (2.0 * sigma12   + C2)
    denominator = (mu1_sq + mu2_sq + C1) * (sigma1_sq + sigma2_sq + C2)

    return numerator / denominator               # [B, 1, H, W]
...
def compute_ssim(img1: torch.Tensor, img2: torch.Tensor,
                 ...
                 data_range: float = 2.0) -> torch.Tensor:
    """
    Mean SSIM per sample in the batch.

    Returns:
        Tensor [B]  ? one SSIM score per image pair
    """
    ssim_map = compute_ssim_map(img1, img2, kernel_size, sigma, data_range)
    return ssim_map.mean(dim=[1, 2, 3])          # [B]


# =============================================================================
# VAE helpers  (same as vae_test.py)
# =============================================================================
...
def evaluate(vae: nn.Module,
    ...
    """
    Run VAE reconstruction and compute per-sample SSIM on the test set.

    Args:
        vae         : AutoencoderKL in eval mode
        test_loader : DataLoader  (NIH, task='test', shuffle=False)
        device      : 'cuda' | 'cpu'
        kernel_size : Gaussian window size for SSIM (default: 11)
    ...
    for imgs, _ in tqdm(test_loader, desc='[SSIM eval]'):
        ...
        # ¦¡¦¡ Per-sample SSIM ¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡
        scores = compute_ssim(imgs, recon,
                              kernel_size=kernel_size,
                              sigma=sigma,
                              data_range=2.0)   # [-1, 1] ¡æ range = 2
...
def parse_args():
    parser = argparse.ArgumentParser(description='VAE SSIM evaluation on NIH test set')
    ...
    parser.add_argument('--kernel_size', default=11,   type=int,
                        help='Gaussian window size for SSIM (default: 11)')
    parser.add_argument('--sigma',       default=1.5,  type=float,
                        help='Gaussian sigma for SSIM (default: 1.5)')
    parser.add_argument('--output_dir',  default='/home/hjchoi/PycharmProjects/PrivaText-CXR/Eval_metric/ssim_vae_test',
...
def main():
    ...
    # ¦¡¦¡ Run evaluation ¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡¦¡
    results = evaluate(vae, test_loader, device,
    ...
    print(f'  SSIM  mean  : {results["mean"]:.4f}')
    print(f'  SSIM  std   : {results["std"]:.4f}')
    print(f'  SSIM  min   : {results["min"]:.4f}')
    print(f'  SSIM  max   : {results["max"]:.4f}')
    print('=' * 45)
