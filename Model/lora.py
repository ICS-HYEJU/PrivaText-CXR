...
Design (docs/DP_LDM_FRAMEWORK_PLAN.md ¡×5):
    ...
    - B  : Linear(r, out_features, bias=False)   ? init: ZERO  (so ¥ÄW=0 at start)
    - scaling = alpha / r
...
class LoRALinear(nn.Module):
    ...
    def __init__(self, base: nn.Linear, rank: int = 4, alpha: float = 4.0,
        ...
        # A: in¡ær, B: r¡æout.  B zero-init ¢¡ ¥ÄW = 0 at initialization.
...
def merge_lora(model: nn.Module) -> int:
    """
    Replace every LoRALinear with its merged plain nn.Linear (W0 + BA) for
