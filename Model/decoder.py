...
class Decoder(nn.Module):
    ...
    def forward(self, z, verbose=False):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                ...
                if verbose:
                    print(f"  L{i_level} upsample      : → {h.shape}")
    ...
    def print_architecture(self):
        ...
        print(f"  conv_in : {self.z_channels} → {block_in}  (3×3 conv)")
        print()
        ...
        print(f"     SiLU")
        print(f"     conv_out  : {block_in} → {self.out_channels}  (3×3 conv)")
        print("=" * 60)
...
def print_level_shapes(encoder: Encoder, decoder: "Decoder", batch_size: int = 1) -> None:
    """
    Run a full encode → sample → decode forward pass and print the output
    ...
    print(f"  (mean + logvar, will be split into 2 × z_channels along dim=1)")
    print()
